...
<div style={{ minHeight: 560, height: "calc(100vh - 220px)" }}>
  <ResizableSplit
    storageKey="shipments:leftWidth"
    defaultLeftPx={520}   // 初期の左幅（好きに）
    minLeftPx={360}       // 最小
    maxLeftPx={900}       // 最大
    left={
      <div style={{ border: "1px solid #ddd", borderRadius: 8, overflow: "hidden", height: "100%" }}>
        ...
        <div style={{ height: "calc(100% - 41px)", overflow: "auto" }}>
          {rows.map((r) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 13, color: "#333", marginTop: 4 }}>{rowTitle(r)}</div>
          ...
          {rows.length === 0 && (
            <div style={{ padding: 12, color: "#666" }}>条件に一致する明細がありません。</div>
          )}
        </div>
      </div>
    }
    right={
      <div style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12, height: "100%", overflow: "auto" }}>
        {/* ====== ここから下は「右の入力フォーム」そのまま ====== */}
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={del} disabled={!form.id || isClosed} style={{ padding: "8px 12px" }}>
              削除
            </button>
          </div>
        </div>
        ...
        {/* ↓↓↓ あなたの既存の入力フォーム（label群）をここにそのまま置く ↓↓↓ */}
        {/* 既存の <div style={{ display:"grid", gridTemplateColumns:"180px 1fr", ... }}> ... </div> を丸ごと貼る */}
      </div>
    }
  />
</div>
...
type ResizableSplitProps = {
  ...
  storageKey?: string;     // 幅の保存キー
  defaultLeftPx?: number;  // 初期幅
  minLeftPx?: number;
  maxLeftPx?: number;
};
...
function ResizableSplit({
  ...
  const [leftWidth, setLeftWidth] = useState<number>(defaultLeftPx);
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (!Number.isNaN(n)) setLeftWidth(clamp(n, minLeftPx, maxLeftPx));
      }
    } catch {}
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  useEffect(() => {
    const onMove = (clientX: number) => {
      ...
      const x = clientX - rect.left; // container左からの位置
      // 左幅は、ドラッグバー中心まで
      setLeftWidth(clamp(Math.round(x), minLeftPx, maxLeftPx));
    };
  ...
  return (
    <div ref={containerRef} className="h-full min-h-0" style={{ height: "100%" }}>
      <div className="grid h-full min-h-0 gap-0" style={gridStyle}>
        {/* 左 */}
        ...
        <div
          ...
          onDoubleClick={() => setLeftWidth(defaultLeftPx)} // ダブルクリックでリセット
          onKeyDown={(e) => {
            if (e.key === "ArrowLeft") setLeftWidth((w) => clamp(w - 20, minLeftPx, maxLeftPx));
            if (e.key === "ArrowRight") setLeftWidth((w) => clamp(w + 20, minLeftPx, maxLeftPx));
            if (e.key === "Enter") setLeftWidth(defaultLeftPx);
          }}
          title="ドラッグで幅調整（ダブルクリックでリセット）"
        >
          {/* 見た目：細い線 + hoverで太く */}
          <div className="absolute inset-y-0 left-1/2 w-px -translate-x-1/2 bg-gray-300/50 group-hover:bg-gray-400/80" />
